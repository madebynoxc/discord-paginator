...
module.exports.create = (options) => {
    ...
    const tick = () => {
        ...
        const now = new Date()
        paginations.filter(x => x.expires < now).map(x => { try {
    ...
    const doSwitch = async (userID, msg, reaction, newpage) => {
        ...
        try {await bot.removeMessageReaction(msg.channel.id, msg.id, reaction, userID)}
        catch(e) {}
    }
    ...
    const remove = async (userID, msg) => {
        ...
        try{await bot.deleteMessage(msg.channel.id, msg.id, 'Removing pagination')}
        catch(e) {}
    }
