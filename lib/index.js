...
module.exports.create = (options) => {
    ...
    const getPages = (array, split = 10, maxCharacters = 1024) => {
        let count = 0, page = 0
        const pages = [""]
        array.map(x => {
            const entry = `${x}\n`
            let newString = pages[page] + entry
            count++

            if(count >= split || newString.length > maxCharacters) {
                page++
                count = 0
                newString = entry
            }

            pages[page] = newString
        })
