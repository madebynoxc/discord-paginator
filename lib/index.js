...
module.exports.create = (options) => {
    ...
    const addPagination = async (userID, channelID, params) => {
        ...
        const obj = Object.assign({
         ...
         if(!obj.embed.color)
            obj.embed.color = colors.pagination

        if(!obj.embed.footer)
            obj.embed.footer = { text: `Page 1/${pages.length}` }
    ...
    const addConfirmation = async (userID, channelID, params) => {
        ...
        if(obj.check && await obj.check())
            return await obj.onError()

        if(params.force)
            return obj.onConfirm()
    ...
    const doSwitch = async (userID, msg, reaction, newpage) => {
        ...
        if(options.wrap && data.pagenum < 0) data.pagenum = max
    ...
    const doResolve = async (userID, msg, reaction) => {
        ...
        if(reaction === chars.decline) 
            await data.onDecline()

        if(data.check && await data.check())
            return await data.onError()
