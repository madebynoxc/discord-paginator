...
module.exports.create = (options) => {
    ...
    const tick = () => {
        ...
        const now = new Date()
        paginations.filter(x => x.expires < now).map(x =>
            bot.removeMessageReactions(x.channel, x.msg).catch())
    ...
    const doSwitch = async (member, msg, reaction, newpage) => {
        ...
        bot.removeMessageReaction(msg.channel.id, msg.id, reaction, userID).catch()
    }
